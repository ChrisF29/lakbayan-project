...
@onready var answer_d = $Panel/VBoxContainer/AnswerD
...
func _ready():
	...
	answer_d.button_down.connect(
		Callable(self, "preview_answer").bind(answer_d, WRONG_COLOR)
	)
	...
	answer_d.pressed.connect(
		Callable(self, "wrong_answer").bind(answer_d)
	)
...
func reset_answer_colors():
	var buttons = [answer_a, answer_b, answer_c, answer_d]
